...
select 2016 as forecastyear
        ...
        protected void btnUpdateForecastUsed_Click(object sender, EventArgs e)
        {
            ...
            Common.runSQLNonQuery(string.Format("update mse_portalforecastreportmanagement set lastfiledate=getdate(),Username='{0}' where reportid=51", HttpContext.Current.User.Identity.Name.ToString()));
        ...
        protected void btnUploadJlpForecast_Click(object sender, EventArgs e)
        {
            string temploadTable = "productdataloader_PortalForecastingData_jlp_tempload";
            string loadTableName = "portalforecastupload_jlp_104_tempload";
            if (fupJlpForecast.HasFile)
            {
                Common.runSQLNonQuery("DELETE FROM " + temploadTable);
                Common.runSQLNonQuery("DELETE FROM " + loadTableName);

                string filename = Path.GetFileNameWithoutExtension(fupJlpForecast.FileName) + "_" + Common.timestamp() + Path.GetExtension(fupJlpForecast.FileName);
                ...
                try
                {
                    try
                    {
                        fupJlpForecast.SaveAs(filePathLocale + filename);
                    }
                    catch
                    {
...
WITH (CODEPAGE = 1252, CHECK_CONSTRAINTS, FIELDTERMINATOR =',', ROWTERMINATOR ='0x0a', FIRSTROW = 2, FIRE_TRIGGERS  ) ", newFilename);
                    ...
                    string updateSQL = string.Format("exec sp_portaljlpforecastupload '{0}','{1}'", fupJlpForecast.FileName, HttpContext.Current.User.Identity.Name.ToString());
                    Common.runSQLNonQuery(updateSQL);
                    ScriptManager.RegisterClientScriptBlock(this.Page, this.Page.GetType(), "alert", "alert('Upload successful, the Jlp forecast has been updated');", true);
                }
                catch (Exception ex)
                {
        ...
        {
            string filename = "Jlp_Forecast_Existing_" + Common.timestamp() + ".csv";
        ...
        {
            string filename = "Jlp_Forecast_Template_" + Common.timestamp() + ".csv";
        ...
        {
            string filename = "Jlp_Forecast_Historic_Year_" + ddlHistoricForecastYears.SelectedValue + "_" + Common.timestamp() + ".csv";
