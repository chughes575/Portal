...
{
    public partial class DixonsDashboard : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!this.IsPostBack)
            {
                ...
                gvDixonsDashboard.DataSource = Common.runSQLDataset("exec [sp_portaldsg_dashboard]");
                gvDixonsDashboard.DataBind();
                if (gvDixonsDashboard.Rows.Count > 0)
                {
                    //Attribute to show the Plus Minus Button.
                    gvDixonsDashboard.HeaderRow.Cells[0].Attributes["data-class"] = "expand";

                    //Attribute to hide column in Phone.
                    gvDixonsDashboard.HeaderRow.Cells[2].Attributes["data-hide"] = "phone";
                    gvDixonsDashboard.HeaderRow.Cells[3].Attributes["data-hide"] = "phone";

                    //Adds THEAD and TBODY to GridView.
                    gvDixonsDashboard.HeaderRow.TableSection = TableRowSection.TableHeader;
                }
            }
        ...
        protected void gvArgosDashboard_PreRender(object sender, EventArgs e)
        {
            ...
            if (gvDixonsDashboard.Rows.Count > 0)
            {
                ...
                gvDixonsDashboard.UseAccessibleHeader = true;
                ...
                gvDixonsDashboard.FooterRow.TableSection = TableRowSection.TableFooter;
            }
        }
        ...
        protected void gvBundleSuggestions_DataBound(Object sender, EventArgs e)
        {
            if (gvDixonsDashboard.Rows.Count > 0)
            {
                gvDixonsDashboard.Enabled = true;
            }
            else
            {
                gvDixonsDashboard.Enabled = false;
            }
            for (int i = 0; i <= gvDixonsDashboard.Rows.Count - 1; i++)
            {

                String status = gvDixonsDashboard.Rows[i].Cells[4].Text;
                String status3PL = gvDixonsDashboard.Rows[i].Cells[5].Text;
                ...
                {
                    case "green":
                        gvDixonsDashboard.Rows[i].Cells[4].BackColor = green;
                        break;
                    case "red":
                        gvDixonsDashboard.Rows[i].Cells[4].BackColor = red;
                        break;
                    case "amber":
                        gvDixonsDashboard.Rows[i].Cells[4].BackColor = amber;
                        break;
                ...
                {
                    case "green":
                        gvDixonsDashboard.Rows[i].Cells[5].BackColor = green;
                        break;
                    case "red":
                        gvDixonsDashboard.Rows[i].Cells[5].BackColor = red;
                        break;
                    case "amber":
                        gvDixonsDashboard.Rows[i].Cells[5].BackColor = amber;
                        break;
        ...
        {
            string filename = "Dixons_Availability_" + Common.timestamp() + ".csv";
            runReport("[sp_portaldsg_dashboard]", filename);
        }
    }
}