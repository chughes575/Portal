...
begin
			...
			select id from @IDNL", customerID);
             string reportID=   Common.runSQLScalar(insertSql).ToString();
                ...
                string insertVendorsSql = string.Format(@"exec sp_jlpVendorInsertVendors {0}", reportID);
                string deleteVendorsSql = string.Format(@"exec sp_jlpVendorDeleteVendors {0}", reportID);
        ...
        protected void gvReportVendors_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            if (commandName == "preview")
            {
                ...
                string spCall = string.Format("exec [sp_JLPVendorPOEmail] {0}", e.CommandArgument.ToString());
...
where jlp.reportid={0} and processed=0", e.CommandArgument.ToString()));
        ...
        protected bool sendVendorEmail(int vendorID, string vendorName, string manufacturer, int reportID)
        {
            ...
            try
            {
                ...
                if (Common.runSQLScalar("select configvalue from portalconfig where configkey='jlpemailtesting'").ToString() == "1")
                {
                    testing = true;
