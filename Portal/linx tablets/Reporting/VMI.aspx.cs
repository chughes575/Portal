...
{
    public partial class VMITest : Page
    {
        ...
        protected void btnUploadVMI_Click(object sender, EventArgs e)
        {
            ...
            {
                Common.runSQLNonQuery("delete from productdataoader_apple_vmi_report_temp_load_new_upload");
                ...
                try
                {
                    ...
                    if (!bypass)
                    {
                        ...
                        string bulkInsert = string.Format(@"BULK INSERT productdataoader_apple_vmi_report_temp_load_new_upload FROM '{0}' 
WITH (CODEPAGE = 1252, CHECK_CONSTRAINTS, FIELDTERMINATOR =',', ROWTERMINATOR ='0x0a', FIRSTROW = 2, FIRE_TRIGGERS  ) ", newFilename);
                        ...
                        if (int.Parse(Common.runSQLScalar("select count(*) from productdataoader_apple_vmi_report_temp_load_new_upload").ToString()) == 0)
                            throw new Exception();

                        string updateSQL = string.Format("exec sp_applevmireport_upload_test '{0}','{1}'", amendedFileName, HttpContext.Current.User.Identity.Name.ToString());
        ...
        protected void btnDownloadVMI_ClickNew(object sender, EventArgs e)
        {
            ...
            this.runReport(string.Format("exec [sp_vmireport_newlayout]  {0} ", (object)num), filename);
        }
        ...
        private void createVmiReport(bool testing)
        {
            ...
            foreach (DataRow dataRow in (InternalDataCollectionBase)Common.runSQLRows(string.Format("select vmireportid,filename,replace(convert(varchar,cast(datecreated as date),103),'/','') from mse_applevmidatareports_test where case when {0} is then 1 else 0coalesce(responsesent,0)=0 order by datecreated asc", (object)Convert.ToInt32(testing))))
            {
