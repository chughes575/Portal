    ...
    public partial class VendorPO : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                bindSuppliers();
                bindLeadTimes();
              string leadtime =Common.runSQLScalar("select configvalue from PortalConfig where ConfigKey='ForecastWeeksUsed' and CustomerID=4").ToString();
        ...
        {
            gvPOSupplierLeadTimes.DataSource = Common.runSQLDataset(@"select * from mse_PortaVendorLeadTimes where customerid=4 order by SupplierDesc");
...
WITH (CODEPAGE = 1252, CHECK_CONSTRAINTS, FIELDTERMINATOR =',', ROWTERMINATOR ='0x0a', FIRSTROW = 2, FIRE_TRIGGERS  ) ", newFilename);
                    ...
                    string updateSQL = string.Format("exec sp_portaluploadproductleadtimes {0},'{1}','{2}'", 4, amendedFileName, HttpContext.Current.User.Identity.Name.ToString());
        ...
        protected void btnRemoveAllLeadTimes_Click(object sender, EventArgs e)
        {
            ...
            if (confirmValue == "Yes")
            {
                Common.runSQLNonQuery("delete from mse_PortaProductLeadTimes where customerid=4");
