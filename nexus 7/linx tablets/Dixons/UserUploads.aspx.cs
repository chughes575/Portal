...
namespace linx_tablets.Dixons
{
    public partial class UserUploads : System.Web.UI.Page
    {
        ...
        public string customerFilename = "Dixons";
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            sqlDSPortal_Dixons_LastUpdates.DataBind();
            gvPortal_Dixons_LastUpdate.DataBind();
        }
        ...
        {
            string tempTableName = "productdataloader_portal_productrange_dixons_tempload";
...
WITH (CODEPAGE = 1252, CHECK_CONSTRAINTS, FIELDTERMINATOR =',', ROWTERMINATOR ='0x0a', FIRSTROW = 2, FIRE_TRIGGERS  ) ", newFilename);
 ...
 where (ExertisProductCode is null)").ToString()) > 0)
                    ...
                    string updateSQL = string.Format("exec sp_portal_dixons_productrange_upload '{0}','{1}',{2}", fuProductRange.FileName, HttpContext.Current.User.Identity.Name.ToString(), customerid);
 ...
 where BundleID=50 order by Product_Description", bundleID));
        ...
        {
            string tempTableName = "portalforecastupload_portal_dixons_tempload";
...
WITH (CODEPAGE = 1252, CHECK_CONSTRAINTS, FIELDTERMINATOR =',', ROWTERMINATOR ='0x0a', FIRSTROW = 2, FIRE_TRIGGERS  ) ", newFilename);
        ...
        {
            string tempTableName = "productdataloader_portalposuppliers_tempload";
...
select count(*) as cnt from productdataloader_portalposuppliers_tempload group by supplierid having count(leadtime)>1) a").ToString()) > 0)
                    ...
                    if (int.Parse(Common.runSQLScalar("select count(*) from productdataloader_portalposuppliers_tempload where supplierid not in (select supplierid from mse_portalposuppliers where customerid=2)").ToString()) > 0)
                        throw new Exception("Unknown SupplierID in file");


                    string updateSQL = string.Format("exec sp_portalvendorleadtime_update '{0}','{1}',{2}", amendedFileName, HttpContext.Current.User.Identity.Name.ToString(), customerid);
                    Common.runSQLNonQuery(updateSQL);
        ...
        {
            string tempTableName = "productdataloader_PortalVendors_dixons_tempload";
...
WITH (CODEPAGE = 1252, CHECK_CONSTRAINTS, FIELDTERMINATOR =',', ROWTERMINATOR ='0x0a', FIRSTROW = 2, FIRE_TRIGGERS  ) ", newFilename);
                    ...
                    string updateSQL = string.Format("exec sp_vendordataPortal_dixons_upload {0},'{1}','{2}'", customerid, fuVendorData.FileName, HttpContext.Current.User.Identity.Name.ToString());
                    Common.runSQLNonQuery(updateSQL);
