...
namespace linx_tablets.SDG
{
    public partial class VendorPO : System.Web.UI.Page
    {
        ...
        protected void btnDLSuggestions_Command(object sender, CommandEventArgs e)
        {
            ...
            switch (commandParam)
            {
                case "1":
                    ...
                    runReport("exec sp_sdgPoSuggestionsAll @vendorname=" +spParam , "POSuggestions_All_" + Common.timestamp() + ".csv");
                    break;
                case "2":
                    if (ddlSuppliers.SelectedIndex != 0)
                        spParam = "'" + ddlSuppliers.SelectedValue + "'";
                    runReport("exec sp_sdgPoSuggestions @vendorname=" + spParam, "POSuggestions_All_" + Common.timestamp() + ".csv");
                    break;
            }
        ...
        {
            gvPOSupplierLeadTimes.DataSource = Common.runSQLDataset(@"select * from mse_PortaVendorLeadTimes order by SupplierDesc");
        ...
        protected void btnProductLeadTimeDownload_Click(object sender, EventArgs e)
        {
            ...
            runReport("select CustomerSku1 as Catno,LeadTime from mse_PortaProductLeadTimes where customerid=1 order by coalesce(CustomerSku1,CustomerSku2)", filename);
        ...
        protected void btnProductLeadTimeUpload_Click(object sender, EventArgs e)
        {
            ...
            {
                Common.runSQLNonQuery("delete from mse_portalproductleadtimes_tempload");
                ...
                try
                {
                    ...
                    string bulkInsert = string.Format(@"BULK INSERT mse_portalproductleadtimes_tempload FROM '{0}' 
WITH (CODEPAGE = 1252, CHECK_CONSTRAINTS, FIELDTERMINATOR =',', ROWTERMINATOR ='0x0a', FIRSTROW = 2, FIRE_TRIGGERS  ) ", newFilename);
                    ...
                    if (int.Parse(Common.runSQLScalar("select count(*) from mse_portalproductleadtimes_tempload").ToString()) == 0)
                        throw new Exception();


                    if (int.Parse(Common.runSQLScalar("select count(*) from mse_portalproductleadtimes_tempload where leadtime is null or coalesce(CustSku,'')=''").ToString()) > 0)
                        throw new Exception();
        ...
        protected void btnRemoveAllLeadTimes_Click(object sender, EventArgs e)
        {
            ...
            if (confirmValue == "Yes")
            {
                Common.runSQLNonQuery("delete from mse_PortaProductLeadTimes");
